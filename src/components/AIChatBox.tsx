        ...
        <div className={cn('bottom-0 right-0 z-10 w-full max-w-[500px] p-1 xl:right-36', open ? "fixed" : "hidden")}>
                ...
                <div className='flex h-[600px] flex-col rounded bg-background border shadow-xl'>
                    ...
                    <form onSubmit={handleSubmit} className='m-3 flex gap-1'>
                        <Button type="button" title='Clear chat' variant="outline" size="icon" className='shrink-0' onClick={() => {
                            setMessages([])
                        }}>
                            <Trash />
                        </Button>
                        <Input value={input} onChange={handleInputChange} placeholder='Say something...' ref={inputRef}/>
...
function ChatMessage({message: {role, content}}: {message: Pick<Message, "role" | "content">}) {
    ...
    return <div className={cn('mb-3 flex items-center', isAiMessage ? "justify-start me-5" : "justify-end ms-5")}>
       ...
       {!isAiMessage &&  user?.imageUrl && (
        <Image src={user.imageUrl} alt='user image' width={100} height={100} className='ml-2 rounded-full w-10 h-10 object-cover'/>
       )}
    </div>
}